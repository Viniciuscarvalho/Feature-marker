...
If `environment_manifest.json` exists in the state directory, extract tool-based constraints:

| MCP/Tool Detected | Constraint                                                                          |
| ----------------- | ----------------------------------------------------------------------------------- |
| XcodeBuildMCP     | "Build verification possible — simulator validation available after implementation" |
| Playwright MCP    | "E2E testing possible — browser-based assertion and visual regression available"    |
| Docker MCP        | "Container build verification possible — image build check available before PR"     |
| swiftlint CLI     | "Swift lint available — modified files will be linted"                              |
| docker CLI        | "Docker available — container builds can be verified"                               |
| gh CLI            | "GitHub CLI available — PR creation automated"                                      |

Add a `### Tool Constraints` sub-heading to `enriched-prompt.md` output listing which tools are available and what they enable.
